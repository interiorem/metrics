...
template<typename Clock>
auto ewma<Clock>::calculate(double value) const -> double {
    return calculate(clock_type::now(), value);
}

template<typename Clock>
auto ewma<Clock>::calculate(time_point time, double value) const -> double {
    if (time < prev) {
