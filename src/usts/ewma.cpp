...
template<typename Clock>
auto ewma<Clock>::get_with(double value) const -> double {
    return get_with(clock_type::now(), value);
}

template<typename Clock>
auto ewma<Clock>::get_with(time_point time, double value) const -> double {
    if (time < prev) {
